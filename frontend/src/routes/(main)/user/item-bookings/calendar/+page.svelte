...
<script>
    ...
    let formattedDevices = $derived(filteredItems ? filteredItems.map((device) => ({
        name: device.name,
        value: device.id,
        description: device.description
    })) : null);

    let formattedDeviceCategories = $derived(deviceCategories ? deviceCategories.map((category) => ({
        name: category.name,
        value: category.id
    })) : []);
    ...
    //             case "laptop":
    //                 return testLaptopTypes;
    //             case "tablet":
    //                 return testTabletTypes;
    //         }
    //     }
    ...
    <div class="flex flex-row gap-4 h-[calc(100svh-79px)]">
        <div class="flex flex-col grow-0">
            <Card shadow={selectedCategory ? false : "md"} class="m-4 p-4">
                ...
                <Select bind:value={selectedCategory} items={formattedDeviceCategories}
                        onchange={() => selectedDevice= ""}></Select>
            </Card>
            {#if selectedCategory}
                ...
                {/if}
                &lt;!&ndash;  <Button class="mt-2">Confirm Reservation</Button> &ndash;&gt;
            {/if}
            &lt;!&ndash;  {/if} &ndash;&gt;
        </div>
