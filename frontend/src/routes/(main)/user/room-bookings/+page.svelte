<script>
    ...
    import {Button, Datepicker, Heading, P, Textarea, Timepicker, Toggle, Tabs, TabItem} from 'flowbite-svelte';
...
<div>
    <Tabs tabStyle="underline" classes={{content: "flex flex-col gap-3 min-h-[calc(100svh-300px)]"}}>
        <TabItem open title="_Calendar View_" classes={{button: 'cursor-pointer'}}>
...
<Button class="cursor-pointer" onclick={saveEvent}
     ...
     <TabItem title="_All Roombookings_" classes={{button: 'cursor-pointer'}} onclick={() => goto('/user/room-bookings/all', {invalidate:['/user/room-bookings/all']})}/>
    </Tabs>
</div>
