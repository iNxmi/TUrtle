...
<div bind:this={multiSelectContainer} onclick={toggleDropdown} onkeydown={handleKeyDown}
    ...
    <span>
        {#if single && selectUsers.length === 1}
            <Badge dismissable={!disabled} color="gray" large={false} params={{duration: 100}}
                   onclose={() => clearOption(selectUsers[0])} class={["mx-0.5 px-2 py-0"]}>
                    {displayFunction(selectUsers[0])}
                </Badge>
        {:else}
            {#each selectUsers as user (user.value)}
                <Badge dismissable={!disabled} color="gray" large={false} params={{duration: 100}}
                       onclose={() => clearOption(user)} class={["mx-0.5 px-2 py-0"]}>
    ...
    <div class="ms-auto flex items-center gap-2">
        ...
        {/if}
        <svg class="ms-1 h-3 w-3 cursor-pointer text-gray-800 dark:text-white" aria-hidden="true"
    ...
    {#if show}
        ...
        <div onclick={(e) => {clickProtected = true; e.stopPropagation()}} role="presentation"
             class={`absolute z-1000 p-3 flex flex-col gap-1 max-h-64 bg-white border border-gray-300 dark:bg-gray-700 dark:border-gray-600 start-0 top-8 cursor-pointer overflow-y-scroll w-full`}>
            <Input onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} size="md"
                   placeholder="_Search User_" type="text" bind:value={searchTerm} {disabled}/>
