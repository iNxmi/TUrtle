<script>
    ...
    async function submit(event) {
        ...
        loading = true;
        const response = await ContentTemplates.create(payload);
        loading = false;
...
<Modal form bind:open={open} outsideclose={false}>
    <form onsubmit={submit} class="flex flex-col gap-5">
        <Heading tag="h3" class="text-center">
            {m.modal_manage_create_content_template_title()}
        </Heading>

        <Hr class="m-0 p-0"/>

        <div>
            <div>{m.modal_manage_create_content_template_label_name()}</div>
            <Input name="name" type="text" bind:value={input.name} required/>
        </div>

        <div>
            <div>{m.modal_manage_create_content_template_label_description()}</div>
            <Input name="description" type="text" bind:value={input.description} required/>
        </div>
        ...
        <Button name="button_submit" type="submit">
            {#if loading === true}
                <Spinner size="5"/>
            {:else}
                {m.modal_manage_create_content_template_button()}
            {/if}
        </Button>
    </form>
</Modal>